...
{   
  ...
  // return $search_col_name;
        foreach ($query_data as $key => $value_q)
        {   
            if(empty($value_q['string']))
            {
...
$query_array='SELECT '.$value_q['get_colms'].' FROM '.$value_q['table_name'].' WHERE  '.$value_q['search_col_name'] .' LIKE "%'.$this->input.'%"'.$append_string_in_sql.'';
